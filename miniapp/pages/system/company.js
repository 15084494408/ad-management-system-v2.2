...
Page({
    ...
    async loadCompanies() {
        ...
        try {
            const res = await (0, request_1.get)('/system/companies', null, { showLoading: false });
            const list = Array.isArray(res) ? res : (res.records || []);
            this.setData({
                companies: list.map((c) => (Object.assign(Object.assign({}, c), { createTime: (0, helpers_1.formatDate)(c.createTime) })))
            });
        }
        catch (e) {
            console.error(e);
    ...
    async submit() {
        ...
        try {
            ...
            if (this.data.editId) {
                await (0, request_1.put)(`/system/companies/${this.data.editId}`, data);
            }
            else {
                await (0, request_1.post)('/system/companies', data);
            }
    ...
    deleteCompany(e) {
        ...
        wx.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {
                    try {
                        await (0, request_1.del)(`/system/companies/${item.id}`);
