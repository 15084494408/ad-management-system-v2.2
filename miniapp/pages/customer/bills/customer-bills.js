...
Page({
    ...
    pickCustomer() {
        const names = this.data.customers.map((c) => c.name);
        names.unshift('全部客户');
        wx.showActionSheet({
            itemList: names,
            success: (res) => {
    ...
    buildParams() {
        const params = { current: this.data.current, size: this.data.size, billType: this.data.billType };
        if (this.data.selectedCustomer)
            params.customerId = this.data.selectedCustomer.id;
        return params;
    },
    resetAndLoad() {
