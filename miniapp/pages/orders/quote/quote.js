...
Page({
    ...
    async loadCustomers() {
        try {
            const res = await (0, request_1.get)('/customers', { current: 1, size: 999 }, { showLoading: false });
    ...
    pickCustomer() {
        const names = this.data.customers.map((c) => c.name || c.customerName);
        if (names.length === 0) {
            wx.showToast({ title: '请先创建客户', icon: 'none' });
            return;
        }
        wx.showActionSheet({
            itemList: names,
    ...
    onQuoteModeChange(e) { this.setData({ quoteMode: e.detail.value }); },
    onLabelChange(e) {
    ...
    calcLabel() {
        ...
        this.setData({
            ...
            totalAmount: (0, helpers_1.formatMoney)(total),
            finalAmount: (0, helpers_1.formatMoney)(Math.max(total - (parseFloat(this.data.discountAmount) || 0), 0))
        });
    },
    calcPrint() {
    ...
    recalcTotal() {
        ...
        this.setData({ finalAmount: (0, helpers_1.formatMoney)(Math.max(total - discount, 0)) });
    },
    async submit() {
        ...
        if (this.data.quoteMode === 'label' && !this.data.labelResult) {
            wx.showToast({ title: '请先计算报价', icon: 'none' });
            return;
        }
        if (this.data.quoteMode === 'print' && !this.data.printResult) {
            wx.showToast({ title: '请先计算报价', icon: 'none' });
            return;
        }
        ...
        try {
            ...
            if (this.data.quoteMode === 'label') {
                ...
                if (r.lineCost > 0)
                    materials.push({ materialName: '划线费', quantity: this.data.label.sheets || 1, unitPrice: parseFloat(this.data.label.linePrice) || 0, amount: r.lineCost });
            }
            else {
                const r = this.data.printResult;
                totalAmount = r.total;
                if (r.bwCost > 0)
                    materials.push({ materialName: '黑白打印', quantity: r.totalBw, unitPrice: r.bwUnitPrice, amount: r.bwCost });
                if (r.colorCost > 0)
                    materials.push({ materialName: '彩色打印', quantity: r.totalColor, unitPrice: r.colorUnitPrice, amount: r.colorCost });
            }
            await (0, request_1.post)('/orders', {
