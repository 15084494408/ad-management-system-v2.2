...
Page({
    ...
    async loadCustomers() {
        try {
            ...
            const list = (res.records || []).map((c) => (Object.assign(Object.assign({}, c), { displayName: c.name || c.customerName || '', typeLabel: c.customerType === 2 ? '工厂' : c.customerType === 3 ? '零售' : '普通' })));
    ...
    async loadDesigners() {
        try {
            const res = await (0, request_1.get)('/system/users', { current: 1, size: 999, designer: 1 }, { showLoading: false });
            this.setData({ designers: (res.records || []).map((u) => ({
                    id: u.id,
    ...
    onCustomerSearch(e) {
        ...
        this.setData({
            filteredCustomers: this.data.customers.filter((c) => (c.displayName || '').toLowerCase().includes(keyword) ||
    ...
    filterMaterials() {
        ...
        if (materialCategoryFilter > 0) {
            list = list.filter((m) => m.categoryId === materialCategoryFilter);
        }
    ...
    addFromLibrary(e) {
        ...
        // 检查是否已添加
        if (this.data.materials.find((s) => s.materialId === m.id && !s.isCustom)) {
    ...
    recalcTotal() {
        ...
        this.setData({
            ...
            finalAmount: (0, helpers_1.formatMoney)(finalVal)
        });
    },
    // ==================== 表单字段 ====================
    onFieldChange(e) {
    ...
    async submit() {
        ...
        if (invalidMat) {
            wx.showToast({ title: '请填写物料名称', icon: 'none' });
            return;
        }
