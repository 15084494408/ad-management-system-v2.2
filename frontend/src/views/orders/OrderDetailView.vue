  ...
  <div class="page-container">
    ...
    <div class="modal-overlay" v-if="previewVisible" @click.self="previewVisible = false">
      <div class="modal-container" style="max-width:700px;">
        ...
        <div class="modal-body">
          <!-- 图片预览 -->
          <div v-if="previewFileData && isImageFile(previewFileData.extension)" class="preview-image-wrap">
            <img :src="'/api/design/file/download/' + previewFileData.id" style="max-width:100%;max-height:60vh;border-radius:8px;" @error="handlePreviewImgError" />
          </div>
          <!-- 非图片文件信息 -->
        ...
        <div class="modal-footer">
          <button v-if="previewFileData?.id" class="btn btn-default" @click="downloadPreviewFile(previewFileData!)">📥 下载文件</button>
...
function downloadFile(file: any) {
  if (file.id) {
    window.open(`/api/design/file/download/${file.id}`, '_blank')
...
function downloadPreviewFile(file: any) {
  downloadFile(file)
}
...
async function deleteFile(file: any) {
  try {
    await ElMessageBox.confirm(`确认删除文件 "${file.originalName || file.name}"？此操作不可恢复。`, '确认删除', {
...
watch(processVisible, async (v) => {
  if (v) {
    // 加载设计师列表
    try {
      const res = await request.get('/system/users/designer-list')
      designerList.value = Array.isArray(res.data) ? res.data : []
    } catch { /* ignore */ }
    // 默认选中当前订单的设计师（等列表加载完再执行）
    await nextTick()
    const currDesignerId = detailData.value?.designerId
    if (currDesignerId) {
      const match = designerList.value.find((d: any) => d.id === currDesignerId || String(d.id) === String(currDesignerId))
      if (match) {
        processForm.designerId = match.id
      }
    }
  } else {
    // 关闭时重置
    processForm.designerId = null
    processForm.deliveryDate = ''
  }
})
