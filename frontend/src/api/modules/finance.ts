...
export const financeApi = {
  /** 财务概览（首页本月收款/待收款/余额等） — 对齐后端 GET /finance/overview */
  getOverview(params?: { startDate?: string; endDate?: string }) {
    return request.get<any, ApiResponse>('/finance/overview', { params })
  },
  ...
  // ── 设计师提成 ──
  /** 设计师列表（下拉框） */
  getCommissionDesigners() {
    return request.get<any, ApiResponse>('/finance/designer-commission/designers')
  },
  /** 提成列表 */
  ...
  /** 批量重算提成 */
  recalcCommission(designerId?: number) {
    return request.post<any, ApiResponse>('/finance/designer-commission/recalc', null, { params: { designerId } })
  },
  /** 设置提成比例并自动重算 */
  setCommissionRate(designerId: number, rate: number, enabled?: boolean) {
    return request.post<any, ApiResponse>('/finance/designer-commission/set-rate', null, {
      params: { designerId, rate, enabled: enabled ?? true }
    })
  },
  /** 批量结算（按设计师） */
  batchSettleCommission(designerId: number) {
    return request.post<any, ApiResponse>('/finance/designer-commission/batch-settle', null, { params: { designerId } })
  },
  /** 批量打款（按设计师） */
  batchPayCommission(designerId: number) {
    return request.post<any, ApiResponse>('/finance/designer-commission/batch-pay', null, { params: { designerId } })
  },
  /** 更新提成记录 */
