...
public class PermissionPatchConfig {
    ...
    private static final List<PermissionDef> PATCHES = Arrays.asList(
        ...
        new PermissionDef(7, "系统配置", "button", "system:config", 12)
    );
    ...
    public CommandLineRunner patchPermissions() {
        return args -> {
            try (Connection conn = dataSource.getConnection();
                ...
                // ★ 始终确保 SUPER_ADMIN 拥有这些关键权限（无论是否新增）
                stmt.executeUpdate(
                    "INSERT IGNORE INTO sys_role_permission (role_id, permission_id) " +
                    "SELECT 1, id FROM sys_permission " +
                    "WHERE permission_code IN ('system:role','system:log','system:dict','system:backup','system:notice','system:menu','system:config','material:view','design:file','statistics:view','square:manage') " +
                    "AND deleted = 0"
                );
                log.info("补丁：已确保 SUPER_ADMIN 拥有全部关键权限");

                if (addedCount > 0) {
                    log.info("补丁：新增了 {} 个权限", addedCount);
                }
