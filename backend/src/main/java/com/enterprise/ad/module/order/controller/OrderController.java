...
public class OrderController {
    ...
    private final DesignerCommissionMapper designerCommissionMapper;
    ...
    public Result<Map<String, Object>> getById(@PathVariable Long id) {
        ...
        // 查询关联的设计文件
        List<DesignFile> designFiles = designFileMapper.selectList(
            new LambdaQueryWrapper<DesignFile>()
                .eq(DesignFile::getOrderId, id)
                .eq(DesignFile::getDeleted, 0)
                .orderByDesc(DesignFile::getCreateTime));
        detail.put("designFiles", designFiles);
     ...
     * 自动计算设计师提成
     * 提成 = 订单总额 × 设计师提成比例
     * 同时写入 fin_designer_commission 表，确保提成管理页面可见
     */
    private void autoCalcCommission(Long orderId, Long designerId, BigDecimal totalAmount) {
        // 先删除旧的提成记录（确保 designerId 为空时清理干净）
        DesignerCommission existing = designerCommissionMapper.selectOne(
            new LambdaQueryWrapper<DesignerCommission>()
                .eq(DesignerCommission::getOrderId, orderId));
        if (existing != null && existing.getDeleted() == 1) existing = null;

        if (designerId == null || totalAmount == null || totalAmount.compareTo(BigDecimal.ZERO) <= 0) {
            // 没有设计师，如果有旧记录则删除
            if (existing != null) {
                designerCommissionMapper.deleteById(existing.getId());
            }
            // 仍更新订单的提数字段为0
            Order clear = new Order();
            clear.setId(orderId);
            clear.setDesignerCommission(BigDecimal.ZERO);
            clear.setUpdateTime(LocalDateTime.now());
            orderMapper.updateById(clear);
            return;
        }
        ...
        // 3. 同步写入/更新 fin_designer_commission 表
        if (existing != null) {
            // 更新已有记录
            existing.setDesignerId(designerId);
            existing.setDesignerName(designerName);
            ...
            designerCommissionMapper.updateById(existing);
        } else {
            // 新建提成记录（状态=1 待结算）
            DesignerCommission dc = new DesignerCommission();
            dc.setOrderId(orderId);
            dc.setOrderNo(orderNo);
            dc.setDesignerId(designerId);
            dc.setDesignerName(designerName);
            dc.setBaseAmount(totalAmount);
            dc.setCommissionRate(rate);
            dc.setCommissionAmount(commission);
            dc.setStatus(1);
            dc.setCreateTime(LocalDateTime.now());
            dc.setUpdateTime(LocalDateTime.now());
            designerCommissionMapper.insert(dc);
        }
    }
}
